...
public class RWTn3270StreamParser
{
   ...
   throws IOException
   {
      for(int i = 0; i < inBufLen; i++)
         //System.out.print(Integer.toHexString(inBuf[i]) + " ");
   ...
   private synchronized void writeOperation()
   {
      if(dataIn[0] == CMD_EAU)
      {
         //System.out.println("Erase all unprotected...");
         eraseAllUnprotected();
         return;
      }
      //now let's check the WCC for bit 0
      if((dataIn[1] & 0x01) != 0)//Bit 7 is set to 1, reset all modified bits
      {
         //System.out.println("Reset MDT");
         resetMDT();
         lastWasCommand = true;
      }
      switch(dataIn[0])
      ...
      if((dataIn[1] & 0x04) != 0)//Bit 5 is set to 1
         beep();
      if((dataIn[1] & 0x02) != 0)//Bit 2 is set to 1
      {
         rw.unlockKeyboard();
   ...
   private synchronized void write()
   {
     ...
     for(counter = 2; counter < dataInLen; counter++)
     {
         ...
         case ORDER_GE:
            ...
            break;
         default:
            display[bufferAddr] = (dataIn[counter] == 0x00)?' ':(char)ebc2asc[dataIn[counter]];
   ...
   private synchronized void eraseAllUnprotected()
   {
      for(int i = 0; i < chars.length; i++)
      {
         ...
         else if(f == null)//not in a field -- unprotected by default
            c.clear();
         //unlock the keyboard
         rw.unlockKeyboard();
   ...
   {
      //System.out.println("Write Structured Field...");
      short queryReply[] = {
       0x88, 0x00, 0x16, 0x81, 0x86, 0x00, 0x08, 0x00, 0xf4, 0xf1, 0xf1, 0xf2, 0xf2, 0xf3, 0xf3, 0xf4,
       0xf4, 0xf5, 0xf5, 0xf6, 0xf6, 0xf7, 0xf7, 0x00, 0x0d, 0x81, 0x87, 0x04, 0x00, 0xf0, 0xf1, 0xf1,
       0xf2, 0xf2, 0xf4, 0xf4, 0x00, 0x17, 0x81, 0x81, 0x01, 0x00, 0x00, 0x50, 0x00, 0x20, 0x01, 0x00,
       0x0a, 0x02, 0xe5, 0x00, 0x02, 0x00, 0x6f, 0x09, 0x0c, 0x0a, 0x00, 0x00, 0x11, 0x81, 0xa6, 0x00,
       0x00, 0x0b, 0x01, 0x00, 0x00, 0x50, 0x00, 0x18, 0x00, 0x50, 0x00, 0x20, 0x00, 0x07, 0x81, 0x88,
       0x00, 0x01, 0x02, 0x00, 0x10, 0x81, 0x85, 0x00, 0x00, 0x09, 0x10, 0x00, 0x00, 0x00, 0x00, 0x03,
       0x00, 0x00, 0xff, 0xff
      };
      
      int		cmnd, length, offset, nleft, pid, sfid, type, buflen;
   	int		i, n;
      buflen = buf.length;
   	offset = 1;
   	nleft = buflen - 1;
   	...
   	while(nleft > 0) {
   		...
   		case 0x01:
   			 ...
   			 */
   			if (length < 5)
   				return;//WSF-RP too small
   
   			pid = buf[offset + 3];
   			type = buf[offset + 4];
   
   			if (type != 0x02)
   				return;//WSF-RP not a query op
   
   			if (pid != 0xFF)
   				return;//WSF-RP invalid PID
   		...
   		case 0x40:
   			 ...
   			 */
   			if (length < 5)
   				return;//WSF-OBDS too small
   
   			pid = buf[offset + 3];
   			cmnd = buf[offset + 4];
   
   			if (pid != 0x00)
   				return;//WSF-OBDS invalid PID
   			...
   			case CMD_EAU_EBCDIC:
   				...
   				break;
   			default:
   				return;//WSF-OBDS unsupported
   			}
   			break;
   		default:
   			return;//unsupported WFS ID
   		}
   ...
   private void beep()
   {
      //TO-DO: Add beep code here... use a callback interface to the consumer.
   ...
   {
      ((RW3270Char)chars[bufferAddr]).clear();//increment the buffer address,
   ...
   private synchronized void startFieldExtended()
   {
      ...
      for(int i = 0; i < pairs; i++)
      {
         ...
         switch(dataIn[++counter])//get the next value from dataIn
   ...
   private synchronized void programTab()
   {
      ...
      if(!lastWasCommand)
      {
         ...
         while(oldAddr < chars.length && !(currChar  = chars[oldAddr]).isStartField())
         {
            currChar.clear();
            oldAddr++;
         }
    ...
    * the data buffer and creating a Field Object for each Start Field character.  The Field
    * objects merely 'point' the the corresponding Start Field character for conceptual ease
    * for end-programmers.  No data is 'contained' in a field object
    */
   private synchronized void buildFields()
   {
      ...
      //now we have to find the end point for the last field.  We can't just set it
      //to the last address in the buffer, because fields that aren't terminated wrap
      //to the beginning of the buffer.
      if(fields.size() > 0)
      {
         RW3270Field firstField = (RW3270Field) fields.elementAt(0);
         lastField.setEnd((firstField.getBegin() == 0)?chars.length:firstField.getBegin() - 1);
   ...
   private short              foreground, background, highlight;
